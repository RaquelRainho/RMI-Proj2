...
def get_param(req):
    try:
        rod_height=0.18
        rod_base_height=0.03
        disk_height=0.04
        gripper_length= 0.1
        disk_handle_offset=[0.07,0.06,0.06]
        number_of_disks=3
        rospy.loginfo("Simulation parameters:\n\tRod height= %s\n\tRod base height= %s\n\tDisk height= %s\n\tGripper length= %s\n\tDisk handle offsets= %s\n\tNumber of disks= %s"%(rod_height, rod_base_height, disk_height, gripper_length, disk_handle_offset,number_of_disks))
        resp = GetParamResponse(rod_height=rod_height, rod_base_height=rod_base_height, disk_height=disk_height, gripper_length=gripper_length, disk_handle_offset=disk_handle_offset, number_of_disks=number_of_disks)
